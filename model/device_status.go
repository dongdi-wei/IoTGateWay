...
type DeviceStatus struct {
	ID				int64       `gorm:"column:id`				//用来对设备数据进行分段的标识
	...
	TTRuntime		int			`gorm:"column:tt_runtime"`		//检测的任务的周期运行时间，单位ms
	...
	Mark			int			`gorm:"column:mark"`			//标记信息，用于计算检测准确率或校验检测模型，0:没有标记,1:正常标记,2:异常标记
